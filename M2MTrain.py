...
def trainwithPER(setID,typ='teacher',learning_rate=1e-4, drop_out=0.4,Layers=[2,1,2], N_hidden=2048, N_cell=2048, steps=20, L2_lambda=1e-4,patience=3, continue_train=0, evalPER=0,lamda=0.5):
    ...
    fname00='%s/%s/LSTMWeight%s/FULL_tea_N2048_D0.3_L0.001_C1024_[2, 2, 2]_S2000_soft_target.npy' %(path,typ,setID)
    ...
    fname='%s/%s/LSTMWeight%s/Pretrain2in1dis_%s_N%s_D%s_L%s_C%s_%s_S%s_l%s' %(path,typ,setID,typ[:3],N_hidden,drop_out,L2_lambda,N_cell,Layers,steps,lamda)
    print(fname)
...
for ll in [6]:
    trainwithPER(str(ll),typ='teacher',learning_rate=9e-5,drop_out=0.3,Layers=[2,1,2,2,2,2], N_hidden=2048, N_cell=1024, steps=2000, L2_lambda=1e-3,patience=3, continue_train=0,evalPER=0,lamda=0.7)
