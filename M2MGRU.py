...
class M2MGRU(object):
    ...
    """
    def __init__(self, learning_rate=1e-4, drop_out=0.3,Layers=[2,1,2,1,2,2], N_hidden=2048, N_cell=2048, D_input=39, D_art=48, D_out=120, Task_type='regression', L2_lambda=0.0, _EPSILON=1e-12,lamda=0.8):
        ...
        self.soft_target = T.matrix('soft_target')
        #art label
        ...
        print('Hard_Softmax')
        self.hard_out_train = T.exp(self.l_out_train)/T.sum(T.exp(self.l_out_train),axis=1, keepdims=True)
        self.hard_out_eval = T.exp(self.l_out_eval)/T.sum(T.exp(self.l_out_eval),axis=1, keepdims=True)
        self.hard_out_train = T.clip(self.hard_out_train, _EPSILON, 1.0 - _EPSILON)
        self.hard_out_eval = T.clip(self.hard_out_eval, _EPSILON, 1.0 - _EPSILON)

        self.hard_loss_train = T.mean(lasagne.objectives.categorical_crossentropy(self.hard_out_train, self.hard_target))
        self.hard_loss_eval = T.mean(lasagne.objectives.categorical_crossentropy(self.hard_out_eval, self.hard_target))
        print('Soft_Softmax')
        self.soft_targets = T.exp(self.soft_target/Tem)/T.sum(T.exp(self.soft_target/Tem),axis=1, keepdims=True)
        self.soft_targets = T.clip(self.soft_targets, _EPSILON, 1.0 - _EPSILON)

        self.soft_out_train = T.exp(self.l_out_train/Tem)/T.sum(T.exp(self.l_out_train/Tem),axis=1, keepdims=True)
        self.soft_out_eval = T.exp(self.l_out_eval/Tem)/T.sum(T.exp(self.l_out_eval/Tem),axis=1, keepdims=True)
        self.soft_out_train = T.clip(self.soft_out_train, _EPSILON, 1.0 - _EPSILON)
        self.soft_out_eval = T.clip(self.soft_out_eval, _EPSILON, 1.0 - _EPSILON)

        self.soft_loss_train = T.mean(lasagne.objectives.categorical_crossentropy(self.soft_out_train, self.soft_targets))
        self.soft_loss_eval = T.mean(lasagne.objectives.categorical_crossentropy(self.soft_out_eval, self.soft_targets))

        #self.total_loss_train=self.hard_loss_train*(1-self.lamda) + self.soft_loss_train*self.lamda
        #self.total_loss_eval=self.hard_loss_eval*(1-self.lamda) + self.soft_loss_eval*self.lamda
        #eval functions
        ...
        #output function
        self.get_out = theano.function([self.l_in.input_var],self.hard_out_eval)
