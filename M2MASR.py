...
class ASR(object):
    ...
    def per_sentence(self, seq_X, seq_Y='0', seq_S='0',steps=20):
        ...
        while idx_present < sen_len:
            if type(seq_Y)!=type('0'):
                lists.append([seq_X[idx_present:idx_present+steps].reshape((1,-1,D_input)).astype("float32")[:,:,:39],np_utils.to_categorical(seq_Y[idx_present:idx_present+steps],D_output).astype("int16"),seq_X[idx_present:idx_present+steps][:,39:].astype("float32"),seq_S[idx_present:idx_present+steps]])
            else:
    ...
    def total_sentences(self,fname,vali_size=10):
        ...
        """
        self.train_data=[]
        for x,y,s in zip(np.load(fname['feature'],encoding='latin1'),np.load(fname['label'],encoding='latin1'),np.load(fname['soft'],encoding='latin1')):
            self.train_data.append(self.per_sentence(x,y,s,self.steps))
